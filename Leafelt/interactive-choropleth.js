function main() {
    var options = {
        center: [46.29518, 8.04795],
        zoom: 14
    };

    var map = L.map('myMap', options);

    // Add the event listener for clicking on the map
    ...
    var selezione = false;
	function processGeoJSON(data, selectedLayer, selezione) {
		var lastMouseOverLayer = null; // Variabile per tenere traccia dell'ultimo layer su cui è stato effettuato il mouseover
		var secondLastMouseOverLayer = null; // Variabile per tenere traccia del secondo ultimo layer su cui è stato effettuato il mouseover
		var geojson = L.geoJson(data, {
			...
			onEachFeature: function(feature, layer) {
				layer.on('click', function(e) {
					...
					var layer = e.target;
					if (!selectedLayer || (selectedLayer && !selezione)) {
						layer.setStyle({
							weight: 3,
							color: 'blue',
							dashArray: '',
				...
				layer.on('mouseover', function(e) {
					...
					//secondLastMouseOverLayer = lastMouseOverLayer; // Aggiorna il secondo ultimo layer su cui è stato effettuato il mouseover
					//lastMouseOverLayer = layer; // Aggiorna l'ultimo layer su cui è stato effettuato il mouseover
					//geojson.resetStyle(secondLastMouseOverLayer);
				});
				...
				layer.on('mouseout', function() {
					...
					// Aggiorna il colore dell'ultimo layer su cui è stato effettuato il mouseover
						if (lastMouseOverLayer) {
							//geojson.resetStyle(secondLastMouseOverLayer);
							lastMouseOverLayer.setStyle({
								weight: 3,
								color: 'blue',
								dashArray: '',
								fillColor: 'blue',
		...
		// Reimposta lo stile di tutti i layer
		function resetAllLayers() {
			geojson.eachLayer(function(layer) {
				geojson.resetStyle(layer);
				lastMouseOverLayer=null;
				info.update();
			});
		}

		// Aggiungi callback per il tasto "Esc"
		function handleEscKeyPress(event) {
			if (event.keyCode === 27) {
				resetAllLayers(lastMouseOverLayer); // Reimposta lo stile di tutti i layer
			}
		}
	...
	function generateMap(data,map) {
		...
		var geojson = processGeoJSON(data,selectedLayer,selezione);
		geojson.addTo(map);
	...
	fetch("https://pizzetti.ch/SchaGaDu_v5_filtered_add.json")
		.then((response) =>{
		...
		function filterGeoJSON(property, value, swisstopo) {
			...
			var geojson = processGeoJSON(filteredFeatures,selectedLayer,selezione);
			geojson.addTo(map);
			//return geojson;
		}
		})
	...
	info.update = function (props) {
		this.div.innerHTML = '<h4>GIS SchaGaDu</h4>' + (props ? 
			...
			props.OBJ_NAME + '</br><br>Link:'+'<a href="https://www.engineering-group.ch/" target="_blank">LINK</a>': 'No object selected');
	};
	info.addTo(map);
	...
	legend.addTo(map);
    // Adding some color, control, and legend...
}
