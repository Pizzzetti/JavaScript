...
function processGeoJSON(data,selectedLayer) {
     // Variabile per tener traccia dello strato selezionato
    var geojson = L.geoJson(data, {
        style: style,
        onEachFeature: function(feature, layer) {
            layer.on('click', function(e) {
                if (!e.layer) {
                    // Clicked outside a shape
                    if (selectedLayer) {
                        selectedLayer.selected = false;
                        info.update(); // Aggiorna la tabella delle informazioni
                        selectedLayer = null; // Imposta selectedLayer a null
                    }
                } else {
                    // Clicked inside a shape
                    if (selectedLayer) {
                        selectedLayer.selected = false; // Deseleziona la forma precedentemente selezionata
                        geojson.resetStyle(selectedLayer); // Reimposta lo stile della forma precedentemente selezionata
                    }

                    // Imposta selectedLayer sul layer cliccato e aggiorna la tabella delle informazioni
            ...
            layer.on('mouseover', function(e) {
                var layer = e.target;
                if (!selectedLayer || (selectedLayer && !selectedLayer.selected)) {
                    layer.setStyle({
                        weight: 3,
                        color: 'blue',
                        dashArray: '',
...
function generateMap(data,map) {
	...
	var geojson = processGeoJSON(data,selectedLayer);
	geojson.addTo(map);
...
fetch("https://raw.githubusercontent.com/Pizzzetti/Public/main/Streamlit/SchaGaDu_v5_filtered_add.json")
	...
	function filterGeoJSON(property, value, swisstopo) {
		...
		var geojson = processGeoJSON(filteredFeatures,selectedLayer);
		geojson.addTo(map);
		//return geojson;
	}
	})
